...
def closest_all_fish_distances(
    ...
    for i, cam in enumerate(cam_names):
        ...
        for tracks_file in tracks_files:
            rl = tracks_crossing_info(tracks_file, 1250)
            dists_dict = defaultdict(list)
            for (tag1, times1), (tag2, times2) in combinations(rl.items(), 2):
        ...
        ax = fig.add_subplot(3, 2, 2 * i + 1, xscale=xscale, yscale=yscale)
        ...
        if not all_distances:
            continue
        ax = fig.add_subplot(3, 2, 2 * i + 2, xscale=xscale, yscale=yscale)
...
def interfish_time_dist_plot(xlog, ylog, altogether, save, line=1250):
    ...
    for i, cam in enumerate(cam_names):
        ...
        interfish_distances = []
        for tracks_file in tracks_files:
            rl = tracks_crossing_info(tracks_file, line)
            all_tags = list(rl)
        ...
        ax = fig.add_subplot(3, 1, i + 1, xscale=xscale, yscale=yscale)
        if xlog:
...
def fish_group_distr_hist(
    ...
    for cam in cam_names:
        ...
        for tracks_file in tracks_files:
            rl = tracks_crossing_info(tracks_file, line)
            groups = dbscan_time_groups(rl, proximity)
            if size1:
...
def make_lollipop_plots(proximity, save, total_plot=False, line=1250):
    ...
    num_rows = 6 if total_plot else 3
    ...
    for i, cam in enumerate(cam_names):
        ...
        for j, tracks_file in enumerate([tracks_files[3], tracks_files[6]]):
            ...
            group_times = []
            for group in groups:
                group_times.append([np.mean(rl[group[0]]) / 3600, len(group)])
            single_times = []
            for single in singles:
                single_times.append([np.mean(rl[single]) / 3600, 1])
    ...
    if save:
        ...
        plt.save_fig(f"group_size_lollipop{last}-{proximity}.jpg", dpi=500)
    plt.show()
...
def gaussian_distr_check(save, line=1250, new_mid=18.5):
    ...
    for cam in cam_names:
        ...
        for tracks_file in [tracks_files[3], tracks_files[6]]:
            rl = tracks_crossing_info(tracks_file, line)
            all_times = np.array([np.mean(times) / 3600 for times in rl.values()])
